...
type IConnection interface {
	...
	SendBuffMsg(msgID uint16, msgType int, data []byte) error //直接将Message数据发送给远程的TCP客户端(有缓冲)
	SendBinaryMsg(msgID uint16, data []byte) error
	SendBinaryBuffMsg(msgID uint16, data []byte) error
